...
func reader(read chan string, scanner *bufio.Scanner) {
	defer close(read)
	for scanner.Scan() {
		read <- scanner.Text()
	}
...
func writer(ch chan string, file *os.File) {
	i := 0
	str := ""
	for {
		data, ok := <-ch
...
func CreateHashFiles(filename string, hashes int) {
	...
	go reader(read, scanner)
	go writer(write, newfile)
